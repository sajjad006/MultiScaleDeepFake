        ...
        └── {Gender}/
            └── {identity_id}/
                └── {video_id}/
                    ...
                    │   └── alignment.json         (WhisperX word+phoneme+syllable boundaries)
                    └── meta.json                  (label, transcript, fps, counts, n_syllables)
...
------------------------
{
  ...
  "language": "en",
  "words": [
    {"word": "hello", "start": 0.0, "end": 0.42, "start_frame": 0, "end_frame": 10}
  ],
  "phonemes": [
    {"phoneme": "HH", "start": 0.0, "end": 0.08, "start_frame": 0, "end_frame": 2}
  ],
  "syllables": [
    {"syl_idx": 0, "syllable": "hel", "word": "hello", "word_pos": "1/2",
     "start": 0.0,  "end": 0.21, "duration_ms": 210.0, "start_frame": 0, "end_frame": 5},
    {"syl_idx": 1, "syllable": "lo",  "word": "hello", "word_pos": "2/2",
     "start": 0.21, "end": 0.42, "duration_ms": 210.0, "start_frame": 5, "end_frame": 10}
  ]
}

  → Syllable lip crops are NOT stored separately.  Use start_frame..end_frame
    to slice directly from scale1_phoneme/lip_crops/ (25fps, every frame present).
...
# ── MediaPipe face mesh loader (supports old and new API) ───────────────────

def load_face_mesh():
    mp = _require("mediapipe")
    try:
        # mediapipe < 0.10 legacy API
...
def get_landmarks(frame_rgb, face_mesh_tuple):
    """
    Returns list of (x_px, y_px) for all 478 landmarks, or None if no face found.
    Handles both legacy and tasks API.
    """
...
# ── syllable extraction (Method B — transcript proportional mapping) ─────────

def _split_word_to_syllables(word: str, dic) -> list:
...
def extract_syllables_from_alignment(word_alignment: list, fps: float) -> list:
    """
    Derive syllable-level timing from WhisperX word-level alignment using
    Method B (transcript proportional mapping).

    Each word's duration is split across its pyphen-derived syllables
    proportionally by character length.  start_frame / end_frame are computed
    the same way as words and phonemes — int(round(t * fps)) — so they index
    directly into scale1_phoneme/lip_crops/.
    ...
    List of dicts with keys:
        syl_idx, syllable, word, word_pos,
        start, end, duration_ms, start_frame, end_frame
    """
    try:
        import pyphen
    except ImportError:
        print("    [WARN] pyphen not installed — syllable extraction skipped. "
    ...
    for word_info in word_alignment:
        ...
        w_end   = float(word_info.get("end",   0.0))
        w_dur   = max(w_end - w_start, 0.01)

        syl_parts = _split_word_to_syllables(word, dic)
        n_syl     = len(syl_parts)
...
def run_whisperx(wav_path: Path, device: str) -> dict:
    try:
        ...
        # ── phoneme alignment via pyfoal (optional) ──────────────────────
        phonemes = []
        try:
            import pyfoal
            for word_seg in words:
                ...
                if not word_text:
                    continue
                start_sample = int(word_seg["start"] * AUDIO_SR)
                end_sample   = int(word_seg["end"]   * AUDIO_SR)
                word_audio   = audio[start_sample:end_sample]
                alignment    = pyfoal.align(word_text, word_audio, AUDIO_SR)
                for phoneme in alignment:
                    phonemes.append({
                        "phoneme": phoneme.phoneme,
                        "start":   round(word_seg["start"] + phoneme.start, 4),
                        "end":     round(word_seg["start"] + phoneme.end,   4),
                    })
        except ImportError:
            pass  # pyfoal not installed — phonemes stay empty
...
def add_frame_indices(alignment: dict, fps: float) -> dict:
    """Add start_frame / end_frame to word and phoneme segments."""
...
def process_video(video_path: Path, video_out: Path, category: str, device: str):
    ...
    video_path structure: .../Category/Ethnicity/Gender/identity_id/video.mp4
    video_out  structure: .../Category/Ethnicity/Gender/identity_id/video_id/
    """
    ...
    if align_path.exists():
        ...
        # back-fill syllables if an older alignment.json lacks them
        if "syllables" not in alignment and alignment.get("words"):
            alignment["syllables"] = extract_syllables_from_alignment(
                alignment["words"], fps
            )
        ...
        # syllables derived from word boundaries — no extra model needed
        alignment["syllables"] = extract_syllables_from_alignment(
            alignment.get("words", []), fps
        )
        with open(align_path, "w") as f:
            json.dump(alignment, f, indent=2)
    ...
    # ── extract identity and other path components ────────────────────────
    ...
    meta = {
        ...
        "n_syllables":     len(alignment.get("syllables", [])),   # ← new
    ...
    for cat in selected_categories:
        ...
        for eth, genders in tree[cat].items():
            ...
            for gen, videos in genders.items():
                ...
                for vp in videos:
                    # ── identity folder is the parent of the video file ──
