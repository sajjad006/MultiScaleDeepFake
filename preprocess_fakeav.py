...
def process_video(video_path: Path, video_out: Path,
    ...
    meta = {
        ...
        "n_phonemes":        len(alignment.get("phonemes",  [])),  # Scale 1
        "n_syllables":       len(alignment.get("syllables", [])),  # Scale 2
        "n_words":           len(alignment.get("words",     [])),  # Scale 3
        "n_gaps":            len(gaps_list),
        "n_vocalizations":   sum(1 for g in gaps_list
                                 if g["type"] == "vocalization"),
        "n_silences":        sum(1 for g in gaps_list
                                 if g["type"] == "silence"),
        "n_lip_frames":      lip_saved,    # Scale 1 (syllable)
        "n_face_frames":     face_saved,   # Scale 2 (word)
        "n_phrase_frames":   phrase_saved, # Scale 3 (phrase)
...
def main():
            ...
            for gen, videos in genders.items():
                ...
                for vp in videos:
                    ...
                    video_out = (output_dir / cat / eth / gen
                                 / identity / vp.stem)
                    queue.append((vp, video_out, cat))

    if not queue:
        sys.exit("[ERROR] No videos matched. Use --list.")

    done = sum(1 for _, vo, _ in queue if video_is_done(vo))
    todo = len(queue) - done
    ...
    for vp, vo, cat in tqdm(queue, desc="Videos", unit="vid"):
        if video_is_done(vo):
            continue
        ...
        try:
            ok = process_video(vp, vo, cat, device)
            if ok:
                success += 1
                ...
                tqdm.write(
                    ...
                    f"lip={m['n_lip_frames']}  "
                    f"face={m['n_face_frames']}  "
                    f"phrase={m['n_phrase_frames']}"
                )
            else:
                failed += 1
                tqdm.write("    FAILED")
